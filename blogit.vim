...
class LocalGMT(datetime.tzinfo):
...
class BlogIt(object):
    ...
    class WordPressBlogPost(BlogPost):
        ...
        def do_send(self, push=None):
            ...
            if push == 0 or self.get_server_var__post_status() == 'draft':
                self.set_server_var__Date_AS_DateTime(DateTime(datetime.datetime.utcnow()))
    ...
    def DateTime_to_str(date, format='%c'):
		...
		gmt_dt = datetime.datetime.fromtimestamp(time.mktime(strptime(str(date), '%Y%m%dT%H:%M:%S')))
		gmt_dt = gmt_dt.replace(tzinfo=gmt)
		gmt_dt = gmt_dt.astimezone(gmt_local)

		return unicode(gmt_dt.strftime(format),
